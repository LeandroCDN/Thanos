...
DISCOVERY_ORDERS = ("volumeNum", "volume24hr", "liquidityNum", "updatedAt")
...
def _build_auth_headers(method: str, path: str, body: str = "") -> dict:
    ...
    api_key    = _env("POLYMARKET_API_KEY", "POLYMARKET_CLOB_API_KEY")
    secret     = _env("POLYMARKET_SECRET", "POLYMARKET_CLOB_SECRET")
    passphrase = _env("POLYMARKET_PASSPHRASE", "POLYMARKET_CLOB_PASSPHRASE")
    address    = _env("POLYMARKET_ADDRESS")
...
def _passes_filters(
    ...
    if end_date_min and end_date and end_date < end_date_min:
        return False
    if end_date_max and end_date and end_date > end_date_max:
        return False
    if volume_min is not None and volume_min > 0 and market["volume"] < volume_min:
        return False
    if liquidity_min is not None and liquidity_min > 0 and market["liquidity"] < liquidity_min:
        return False
    return True
...
async def fetch_all_markets(
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for order in DISCOVERY_ORDERS:
            ...
            for _ in range(max_pages):
                ...
                resp = await client.get(MARKETS_ENDPOINT, params=params)
                if resp.status_code == 422 and markets_by_id:
                    break
                resp.raise_for_status()
                data = resp.json()
    ...
    return sorted(markets_by_id.values(), key=lambda m: m["volume"], reverse=True)
...
async def fetch_balance() -> dict:
    ...
    secret         = _env("POLYMARKET_SECRET", "POLYMARKET_CLOB_SECRET")
    passphrase     = _env("POLYMARKET_PASSPHRASE", "POLYMARKET_CLOB_PASSPHRASE")
    proxy          = _env("POLYMARKET_PROXY", "POLYMARKET_FUNDER_ADDRESS")
    private_key    = _env("POLYMARKET_PRIVATE_KEY")
    signature_type = _env("POLYMARKET_SIGNATURE_TYPE") or "0"

    if private_key:
        balance = _fetch_balance_with_clob_client(private_key, proxy, signature_type)
...
def _signature_type_candidates(configured: str) -> list[int]:
    candidates = []
    for raw in (configured, "2", "1", "0", "3"):
        try:
            value = int(raw)
...
def _fetch_balance_with_clob_client(private_key: str, funder: str, signature_type: str) -> dict:
    """Use the official CLOB client to derive API creds from the signer key."""
    try:
    ...
    last_error = ""
    zero_balance: dict | None = None

    for sig_type in _signature_type_candidates(signature_type):
        try:
            client = ClobClient(
    ...
    if zero_balance is not None:
        return zero_balance
    return {"error": f"CLOB auth failed: {last_error[:100]}"}
